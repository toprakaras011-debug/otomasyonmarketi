...
import { signUp, signUpWithOAuth } from '@/lib/auth';
...
export default function SignUpPage() {
  ...
  const handleOAuthSignUp = async (provider: 'google' | 'github') => {
    try {
      setLoading(true);
      logger.debug('OAuth sign up initiated', { provider });
      
      await signUpWithOAuth(provider, '/dashboard');
      
      // OAuth redirect will happen automatically
      // No need to handle redirect here as Supabase handles it
    } catch (error: any) {
      logger.error('OAuth sign up error', error);
      toast.error(error?.message || `${provider === 'google' ? 'Google' : 'GitHub'} ile kayıt olunamadı. Lütfen tekrar deneyin.`, {
        duration: 5000,
      });
      setLoading(false);
    }
